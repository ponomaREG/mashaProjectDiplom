...
class Order:
    ...
    def getOrdersOfUser(userID):
        ...
        if(allRows is None or len(allRows) == 0):
            ...
            result['data'] = []
            return result
        query = 'select SUM(price*count) as "total" from Забронированная_книга where order_id = {};'
        for row in allRows:
            rowWithTotalOfOrder = SqlExecuter.getOneRowsPacked(query.format(row['id']))
            row['total'] = rowWithTotalOfOrder['total']
        result['status'] = 0
        result['message'] = 'OK'
        result['data'] = allRows
        return result
    ...
    def getDetailsOfOrder(userID,orderID):
        ...
        data = SqlExecuter.getAllRowsPacked("select pr.id,pr.imageLink,pr.book_title,bk.count,bk.price*bk.count as 'total' from Забронированная_книга as bk inner join Товар as pr on bk.order_id = {} and pr.id = bk.product_id;".format(orderID))
        for row in data:
            row['imageLink'] = imageHelper.makeFullPathToImage(row['imageLink'])
        rowAddressPacked = SqlExecuter.getOneRowsPacked(
            "select ord.id as 'orderID',addr.*,ord.date,ord.status,ord.total,sum(book.rpice*book.price) as 'total',concat(addr.district,' district,',addr.street,', ',addr.house) as 'address' from Заказ as ord  inner join Адрес as addr inner join Забронированная_книга as book \
    ...
    def addNewOrder(userID,district,flat,house,floor,street,porch='',email='',desc = '',phone=''):
        ...
        try:
            lastrowid = SqlExecuter.executeModif('insert into Заказ(`user_id`,`status`,`address_id`,`description`,`email`,`phone`) values({},{},{},"{}","{}","{}");'.format(
                    userID,
                    0,
                    lastrowidAddress,
                    desc,
